#!/usr/bin/env bash
#SBATCH --job-name=eventmatr-d16-risk-smoke
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
export PYTHONPYCACHEPREFIX="${MATR_OUTPUT_ROOT}/pycache/d16-risk-smoke"

MANIFEST="${PROJECT_DIR}/experiment_configs/eventmatr_d1_preexperiments.json"
python3 scripts/verify_source_identity.py \
  --project-dir "${PROJECT_DIR}" \
  --manifest "${MANIFEST}" \
  --output "${MATR_OUTPUT_ROOT}/source_identity_d16_smoke_start.json" \
...
python3 scripts/run_eventmatr_d16_risk_smoke.py \
  ...
  --output "${MATR_D16_SMOKE_RECEIPT}"

python3 scripts/verify_source_identity.py \
  --project-dir "${PROJECT_DIR}" \
  --manifest "${MANIFEST}" \
  --output "${MATR_OUTPUT_ROOT}/source_identity_d16_smoke_final.json" \
  --expected-commit "${MATR_SOURCE_COMMIT}" \
  --expected-tree "${MATR_SOURCE_TREE}" \
  --expected-manifest-sha256 "${MATR_MANIFEST_SHA256}" \
