...
if [[ "${MATR_LANE:-}" != "TH" ]]; then
  echo "D1.4 mechanism runs are restricted to MATR_LANE=TH" >&2
  exit 2
fi
...
if [[ -e "${LOCKED_TEST_SENTINEL}" ]]; then
  echo "locked-test sentinel unexpectedly exists before D1.4 training" >&2
  exit 2
fi
ARGS=(
  ...
  --event_d14_variant "${MATR_D14_VARIANT}"
