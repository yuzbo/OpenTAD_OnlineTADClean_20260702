#!/usr/bin/env bash
#SBATCH --job-name=eventmatr-d13-mechanism
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
: "${MATR_D13_VARIANT:?MATR_D13_VARIANT is required}"
...
case "${MATR_D13_VARIANT}" in
  soft_assignment_only|event_matched_birth_only|combined) ;;
  *)
    echo "invalid MATR_D13_VARIANT=${MATR_D13_VARIANT}" >&2
    exit 2
    ;;
esac

RUN_ROOT="${MATR_MECHANISM_ROOT}/${MATR_D13_VARIANT}/TH"
mkdir -p "${RUN_ROOT}"
...
bash scripts/train_eventmatr_d13_mechanism.sh

python3 scripts/finalize_eventmatr_d13_mechanism.py \
  --output-root "${RUN_ROOT}" \
  --variant "${MATR_D13_VARIANT}" \
