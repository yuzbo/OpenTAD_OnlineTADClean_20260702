#!/usr/bin/env bash
#SBATCH --job-name=eventmatr-d14-mechanism
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
case "${MATR_D14_VARIANT}" in
  normalized_survival|decision_aligned_bag) ;;
  *)
    echo "invalid MATR_D14_VARIANT=${MATR_D14_VARIANT}" >&2
    exit 2
    ;;
esac

RUN_ROOT="${MATR_MECHANISM_ROOT}/${MATR_D14_VARIANT}/TH"
mkdir -p "${RUN_ROOT}"
...
bash scripts/train_eventmatr_d14_mechanism.sh
...
python3 scripts/finalize_eventmatr_d14_mechanism.py \
  --output-root "${RUN_ROOT}" \
  --variant "${MATR_D14_VARIANT}" \
