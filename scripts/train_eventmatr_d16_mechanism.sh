...
: "${MATR_D16_ARM:?MATR_D16_ARM is required}"

case "${MATR_D16_ARM}" in
  control) D16_VARIANT=none ;;
  risk) D16_VARIANT=policy_independent ;;
  *)
    echo "invalid MATR_D16_ARM=${MATR_D16_ARM}" >&2
    exit 2
    ;;
esac

if [[ "${MATR_LANE:-}" != "TH" ]]; then
  echo "D1.6 mechanism runs are restricted to MATR_LANE=TH" >&2
  exit 2
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOCKED_TEST_SENTINEL="${MATR_OUTPUT_ROOT}/LOCKED_TEST_NOT_MOUNTED.pickle"
if [[ -e "${LOCKED_TEST_SENTINEL}" ]]; then
  echo "locked-test sentinel unexpectedly exists before D1.6 training" >&2
  exit 2
fi
