...
def test_formal_run_manifest_requires_trusted_signature_and_artifact_hashes(tmp_path):
    ...
    assert verify_formal_run_manifest(signed, trust_root=trust_root)["seed"] == 705

    forged = json.loads(json.dumps(signed))
    forged["seed"] = 706
    with pytest.raises(TrainingEvidenceError, match="signature"):
        verify_formal_run_manifest(forged, trust_root=trust_root)

    artifact.write_text('{"locked":false}\n', encoding="utf-8")
    with pytest.raises(TrainingEvidenceError, match="hash differs"):
        verify_formal_run_manifest(signed, trust_root=trust_root)
