...
from opentad.evaluations.online_instance_metrics import OnlineInstanceProtocolError
...
def test_metrics_keep_duplicate_fragmented_and_unmatched_emissions_auditable():
    ...
    metrics = compute_online_instance_metrics(ground_truth, emissions, tiou_threshold=0.5)
...
def test_explicit_frame_bounds_take_precedence_over_display_segment_units():
    ...
    metrics = compute_online_instance_metrics(ground_truth, emissions)
...
def test_causal_validation_rejects_future_tainted_or_mutable_rows(row, violation):
    with pytest.raises(OnlineInstanceProtocolError, match=violation):
        compute_online_instance_metrics([], [row])
...
def test_causal_validation_rejects_nonmonotonic_stream_order(emissions, violation):
    with pytest.raises(OnlineInstanceProtocolError, match=violation):
        compute_online_instance_metrics([], emissions)
...
def test_lifecycle_metrics_count_slot_survival_and_target_binding_shuffles():
    ...
    metrics = compute_online_instance_metrics([], [], lifecycle_traces=lifecycle_traces)
