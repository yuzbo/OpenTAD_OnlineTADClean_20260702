...
Do not start profile or training. Freeze the `70df86e` terminal G0 `KILL` and use its four exposed samples only for zero-GPU development diagnosis. Round 1 has now established a structural omitted-history defect in the current empty-state replay and resolved U1-U8 from the local signed bundle. The next gate is same-reviewer Round 2 over three state-faithful families: full causal forward plus selective backward, exact chronological recomputation, and explicit state checkpoints. It must also specify a capacity-safe Q2 reference and the minimum unseen multi-checkpoint G0. Any retained route requires a new clean commit, unseen holdout preregistration, full B0, independent review, and terminal G0 PASS before the next profile GPU-hour.
